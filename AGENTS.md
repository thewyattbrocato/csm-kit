...
- **Determinism:** identical inputs + same `--as-of` must produce a byte-identical brief. Never read wall-clock time outside the as-of default in `bin/csmkit.js`.
...
- `MEETING_RE` uses word boundaries deliberately: substring matching classified activity types like `ticket_review` as meetings.
