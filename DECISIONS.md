...
**Choice:** (b). Above the fold, in order: typographic banner (`banner.svg`) → one-paragraph value prop → flow diagram (`how-it-works.svg`, annotating fail-closed + completeness score) → verbatim excerpt of real generated output with inline `file#L<row>` citations → before/after callout (D17 later inserts the runnable Quickstart before this callout) → badges row (CI, tests, license, node). All existing PM-artifact depth (problem statement, discovery evidence, roadmap) stays intact below the fold. Every image is a hand-built SVG using system monospace font stacks — zero external fonts, images, or services; each badge links to where its claim is verifiable (live GitLab pipelines, in-repo sources).
...
## D17. Quickstart one scroll under the proof, prerequisites in prose, install forms on equal footing

**Context:** The fresh-eyes usability audit found the first runnable command buried at README line ~97 behind the problem essay and hero material, and the Node ≥ 18 requirement stated only in badge alt-text — a stranger on old Node hits an unexplained failure with no hint.

**Alternatives:** (a) Quickstart as the very first section, before any proof; (b) Quickstart immediately after the Real output excerpt, before the before/after callout; (c) status quo — commands live only inside each brief-type section.

**Choice:** (b). Above-the-fold order becomes banner → value prop → flow diagram → real-output excerpt → **Quickstart** → before/after → badges. The section states the prerequisite as prose ("Requires Node ≥ 18 … zero dependencies, no network calls"), then clone → install → one renewal run with its exact expected output line, then the two sibling types and `--stats`, ending in a pointer to Input schemas. Global `npm install -g .` (sudo caveat named) and the zero-install `node bin/csmkit.js` form are presented on equal footing rather than install-or-footnote. Per-type sections keep their full self-contained console blocks; the Quickstart reuses those commands verbatim instead of paraphrasing variants.

**Why:** The tool's best argument is its artifact, so proof stays first — but proof without a next step wastes the visitor's momentum, and setup clarity was the audit's single blocker. Prerequisites belong in prose because badges don't fail fast for the reader. Keeping Quickstart commands byte-identical to the per-type blocks keeps the stranger test cheap: every printed command can be copied literally from a clean clone and must succeed.

> **Pattern:** Put the how within one scroll of the why, and treat printed commands as a runnable contract — prerequisites in prose, alternatives side by side, nothing that fails when copied literally.
