...
function questionsRowsAsOf(questions, asOfDt) {
  return questions.rows.filter((q) => q.dt.getTime() <= asOfDt.getTime());
}

// ---------- completeness ----------

function computeHandoffCompleteness(handoff) {
  const units = [];
...
function gapReportItems(completeness, handoff, questions, questionRows) {
  const items = [];
...
function buildHandoffBrief({ handoff, crm, questions, asOfDt }) {
  const completeness = computeHandoffCompleteness(handoff);
  const visibleQuestions = questionsRowsAsOf(questions, asOfDt);
  const risks = handoffRiskFlags(handoff, crm, asOfDt);
  const gaps = gapReportItems(completeness, handoff, questions, visibleQuestions);
