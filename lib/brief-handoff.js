...
const { fmtInt, citeSpan, mdText, renderCompleteness } = require('./render');
...
function handoffRiskFlags(handoff, crm, asOfDt) {
  ...
  if (crm.rows.length > 0) {
    ...
    for (const s of unmatched.slice(0, 3)) {
      pushFlag(
        `🟠 No CRM-recorded activity for mapped stakeholder "${mdText(s.name)}"${s.role ? ` (${mdText(s.role)})` : ''} as-of ${isoOf(asOfDt)} — search covered \`${crm.rangeSpan}\``
      );
    }
    if (unmatched.length > 3) {
      pushFlag(
        `🟠 ${unmatched.length - 3} additional mapped stakeholder${unmatched.length - 3 === 1 ? '' : 's'} with no CRM-recorded activity — search covered \`${crm.rangeSpan}\``,
        unmatched.length - 3
      );
    }
  }
...
function crmTouchFor(stakeholder, crm, asOfDt) {
  ...
  return { count: matches.length, last };
}
  ...
  if (handoff.stakeholders.length > 0) {
    ...
    if (crm.rows.length > 0) {
      ...
      for (const s of handoff.stakeholders) {
        ...
        out.push(
          `| ${mdText(s.name)} | ${s.role ? mdText(s.role) : '—'} | ${s.handoffRole ? mdText(s.handoffRole) : '—'} | ${lastCell} | ${fmtInt(touch.count)} | \`${handoff.source}#L${s.line}\` |`
        );
      }
    } else {
