...
function refList(source, lines, max = 8) {
  if (lines.length <= max) return citeSpan(source, lines);
  const shown = lines.slice(0, max).map((l) => 'L' + l).join(',');
  return '`' + source + '#' + shown + '` +' + (lines.length - max) + ' more';
}
...
function computeCompleteness(account, crm, tickets, usage) {
  ...
  push(
    ...
    renewOk,
    renewOk ? '`' + account.spans.renewalDate + '`' : 'missing or invalid',
...
function signalRows(crm, tickets, usage, asOfDt) {
  ...
  if (crm.rows.length > 0) {
    const last = crm.rows[crm.rows.length - 1];
    const who = last.contact ? ` with ${last.contact}` : '';
    rows.push({
      signal: 'Last logged activity',
      value: `${last.date} — ${last.type || 'unspecified type'}${who}`,
      source: citeSpan(crm.source, [last.line]),
    });

    const msDay = 86400000;
    const recent = crm.rows.filter((r) => asOfDt.getTime() - r.dt.getTime() < 30 * msDay);
    const prior = crm.rows.filter((r) => {
    ...
    rows.push({
      ...
      source: refList(crm.source, [...recent, ...prior].map((r) => r.line)),
    });
  }

  if (tickets.rows.length > 0) {
    const open = tickets.rows.filter((t) => t.open);
    const bySev = (level) => open.filter((t) => t.severity === level).length;
    const value =
      open.length === 0
        ? `0 open (${tickets.rows.length} resolved on record)`
        : `${open.length} open (${bySev('high')} high · ${bySev('medium')} medium · ${bySev('low')} low)`;
    const cited = open.length > 0 ? open.map((t) => t.line) : tickets.rows.map((t) => t.line);
    rows.push({ signal: 'Ticket load', value, source: refList(tickets.source, cited) });
  }

  if (usage.rows.length > 0) {
    const latest = usage.rows[usage.rows.length - 1];
    const parts = [];
    ...
    if (parts.length > 0) {
      rows.push({
        signal: `Latest usage period (${latest.period})`,
        value: parts.join(' · '),
        source: citeSpan(usage.source, [latest.line]),
      });
    }
    const firstWithUsers = usage.rows.find((r) => r.activeUsers !== null);
    if (firstWithUsers && latest.activeUsers !== null && usage.rows.length >= 2) {
      const delta = Math.round(((latest.activeUsers - firstWithUsers.activeUsers) / firstWithUsers.activeUsers) * 100);
      rows.push({
        signal: `Usage trend (active users, ${firstWithUsers.period} → ${latest.period})`,
        value: `${delta >= 0 ? '+' : ''}${delta}%`,
...
function riskFlags(account, crm, tickets, usage, asOfDt, countdownDays) {
  ...
  const staleHigh = tickets.rows.filter(
    (t) => t.open && t.severity === 'high' && daysBetween(t.openedDt, asOfDt) > 14
  );
  for (const t of staleHigh.slice(0, 3)) {
    const age = daysBetween(t.openedDt, asOfDt);
    flags.push(
      `🔴 High-severity ticket open ${age} days: "${t.subject || 'untitled'}" — ${citeSpan(tickets.source, [t.line])}`
    );
  }
  ...
  if (usage.rows.length >= 2) {
    ...
    if (
      ...
      if (dropPct >= 20) {
        flags.push(
          `🟠 Usage declining: active users down ${Math.round(dropPct)}% (${firstWithUsers.period} → ${latest.period}) — ${citeSpan(usage.source, [firstWithUsers.line, latest.line])}`
        );
      }
    }
  }

  if (countdownDays !== null && countdownDays <= 60 && crm.present) {
    const meetings = crm.rows.filter((r) => MEETING_RE.test(r.type));
    ...
    if (recentMeeting) {
      ...
      flags.push(
        `🔴 Renewal within 60 days but no customer meeting/call in the last 30 days (last: ${lastMeeting.date}) — renewal \`${account.spans.renewalDate}\`, last meeting ${citeSpan(crm.source, [lastMeeting.line])}`
      );
    } else {
      flags.push(
  ...
  if (countdownDays !== null && countdownDays < 0) {
    flags.push(`🔴 Renewal date has already passed — confirm actual date — \`${account.spans.renewalDate}\``);
  }

  return flags;
}

function stakeholderMap(crm) {
  const byContact = new Map();
  for (const r of crm.rows) {
    const key = (r.contact || '').trim();
    if (!key) continue;
    const cur = byContact.get(key) || { name: key, role: '', last: null, count: 0 };
    cur.count += 1;
    if (!cur.last || r.dt.getTime() >= cur.last.dt.getTime()) cur.last = r;
    if (r.role) cur.role = r.role;
...
function buildBrief({ account, crm, tickets, usage, asOfDt }) {
  ...
  const stakeholders = crm.rows.length > 0 ? stakeholderMap(crm) : [];

  const out = [];
  const title = account.fields.name ? account.fields.name.raw : 'Unnamed account';
  out.push(`# Renewal Readiness Brief — ${title}`);
  out.push('');
  ...
  if (risks.length > 0) {
    ...
    out.push('## Risk flags');
    out.push('');
    out.push('_None triggered by the current deterministic rules._');
    out.push('');
  }
  ...
  if (stakeholders.length > 0) {
    ...
    for (const s of stakeholders) {
      out.push(
        `| ${s.name} | ${s.role || '—'} | ${s.last.date} (${s.last.type || 'n/a'}) | ${s.count} | ${citeSpan(crm.source, [s.last.line])} |`
      );
    }
    out.push('');
  }
  ...
  if (!account.fields.arrUsd) recommended.push('`arr_usd:` in account.yaml (recommended — sizes commercial urgency)');

  out.push('## Missing-evidence checklist');
  out.push('');
  if (missing.length === 0 && recommended.length === 0) {
    out.push('- [x] All required evidence present and all recommended fields filled.');
  }
