...
function evidenceRangeList(...exports) {
  return exports
    .filter((e) => e.rows.length > 0 && e.rangeSpan)
    .map((e) => '`' + e.rangeSpan + '`')
    .join(', ');
}
...
function signalRows(crm, tickets, usage, asOfDt) {
  ...
  if (crm.rows.length > 0) {
    ...
    const recent = crm.rows.filter((r) => {
  ...
  if (usage.rows.length > 0) {
    const usageRows = usageChronologicalRows(usage);
...
function riskFlags(account, crm, tickets, usage, asOfDt, countdownDays) {
  ...
  if (countdownDays !== null && countdownDays <= 60 && crm.rangeSpan) {
    ...
    const meetingsAsOf = meetings.filter((r) => asOfDt.getTime() - r.dt.getTime() >= 0);
    const recentMeeting = [...meetingsAsOf].reverse().find((r) => asOfDt.getTime() - r.dt.getTime() < 30 * msDay);
    if (recentMeeting) {
      // Near renewal WITH a recent meeting: not a flag.
    } else if (meetingsAsOf.length > 0) {
      const lastMeeting = meetingsAsOf[meetingsAsOf.length - 1];
      flags.push(
...
function stakeholderMap(crm) {
  ...
  for (const r of crm.rows) {
    ...
    if (!key) continue;
    const cur = byContact.get(key) || { name: key, role: '', roleLine: null, last: null, count: 0 };
    cur.count += 1;
...
function buildBrief({ account, crm, tickets, usage, asOfDt }) {
  ...
  if (risks.length > 0) {
    ...
    out.push('## Risk flags');
    out.push('');
    out.push(`_None triggered by the current deterministic rules._ Evidence checked: ${evidenceRangeList(crm, tickets, usage)}.`);
    out.push('');
  }
  ...
  if (stakeholders.length > 0) {
    ...
    for (const s of stakeholders) {
      const sourceLines = s.roleLine ? [s.roleLine, s.last.line] : [s.last.line];
      out.push(
        `| ${s.name} | ${s.role || '—'} | ${s.last.date} (${s.last.type || 'n/a'}) | ${s.count} | ${citeSpan(crm.source, sourceLines)} |`
      );
    }
    out.push('');
  }
