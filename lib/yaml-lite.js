...
function parseFlatYaml(text) {
  ...
  lines.forEach((raw, idx) => {
    ...
    let line = raw;
    if (/^\t/.test(line)) {
      errors.push(`account.yaml#L${lineNo}: tab indentation is not supported; use spaces`);
      return;
    }
    ...
    if (/^-\s/.test(trimmed)) {
      errors.push(`account.yaml#L${lineNo}: list items are not supported in v0.1 (flat keys only)`);
      return;
    }
    ...
    if (!match) {
      errors.push(
        `account.yaml#L${lineNo}: expected "key: value" (nesting/multi-line values are not supported in v0.1)`
      );
      return;
    }
    const key = match[1];
    ...
    if (value === '') {
      errors.push(`account.yaml#L${lineNo}: empty value for "${key}"`);
