...
function loadTickets(filePath) {
  ...
  for (const row of table.rows) {
    ...
    if (statusClosed && closedIso === null) {
  ...
  for (const row of table.rows) {
    ...
    const num = (key) => {
      ...
      if (!Number.isFinite(n)) {
        warnings.push(`${name}#L${row.line}: ignored non-numeric ${key} "${cell(row, map, key)}"`);
        return null;
      }
      if (!Number.isInteger(n) || n < 0) {
        warnings.push(`${name}#L${row.line}: ignored invalid ${key} "${cell(row, map, key)}" (must be a non-negative integer)`);
        return null;
      }
      return n;
    };
    rows.push({ line: row.line, period, periodKey, activeUsers: num('active_users'), events: num('events') });
  }
  return {
    present: true,
    empty: rows.length === 0,
