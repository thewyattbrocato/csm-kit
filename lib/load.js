...
function csvWarnings(name, table) {
  return (table.errors ?? []).map((e) => `${name}#L${e.line}: ${e.message}`);
}

function loadAccount(filePath) {
  ...
  if (renewalEntry) {
    ...
    if (!dt) {
      errors.push(
        `account.yaml#L${renewalEntry.line}: renewal_date "${renewalEntry.raw}" is not a valid YYYY-MM-DD date`
      );
    } else {
  ...
  if (arrEntry && !/^\d+(\.\d+)?$/.test(arrEntry.raw.replace(/[, _]/g, ''))) {
    errors.push(`account.yaml#L${arrEntry.line}: arr_usd "${arrEntry.raw}" is not a number`);
    arrUsd = null;
  }
  ...
  return {
    ...
    spans: {
      ...
      owner: span('owner'),
      tier: span('tier'),
      arrUsd: arrUsd ? span('arr_usd') : null,
...
function notProvided() {
  return { present: false, empty: true, source: null, rows: [], rangeSpan: null, warnings: [] };
}
...
function loadCrm(filePath) {
  if (!filePath) return notProvided();
  const name = baseName(filePath);
  const text = readTextOrNull(filePath, 'CRM activity');
  const table = parseCsvTable(text);
  if (table.header.length === 0) {
    return { present: true, empty: true, source: name, rows: [], rangeSpan: null, warnings: csvWarnings(name, table) };
  }
  ...
  for (const row of table.rows) {
    ...
    if (!dt) {
      warnings.push(`${name}#L${row.line}: skipped — date "${dateRaw}" is not YYYY-MM-DD`);
      continue;
    }
    rows.push({
      line: row.line,
...
function loadTickets(filePath) {
  ...
  for (const row of table.rows) {
    ...
    if (!openedDt) {
      warnings.push(`${name}#L${row.line}: skipped — opened "${openedRaw}" is not YYYY-MM-DD`);
      continue;
    }
    let closedIso = null;
    let closedDt = null;
    ...
    if (closedRaw !== '') {
      ...
      if (!closedDt) {
        warnings.push(`${name}#L${row.line}: ignored closed "${closedRaw}" (not YYYY-MM-DD)`);
      } else {
        if (closedDt.getTime() < openedDt.getTime()) {
          closedWarning = `${name}#L${row.line}: ignored closed "${closedRaw}" before opened "${openedRaw}"`;
          closedDt = null;
        } else {
    ...
    if (closedWarning) {
      ...
      warnings.push(`${name}#L${row.line}: ${reason}; closed status is not treated as resolved as-of`);
    }
    ...
    if (severityRaw !== '' && severity === null) {
      warnings.push(`${name}#L${row.line}: severity "${severityRaw}" is unrecognized; counted as unknown`);
    }
    rows.push({
      line: row.line,
...
function loadUsage(filePath) {
  ...
  for (const row of table.rows) {
    ...
    if (period === '') {
      warnings.push(`${name}#L${row.line}: skipped — empty period`);
      continue;
    }
    const periodKey = parseUsagePeriodKey(period);
    if (periodKey === null) {
      warnings.push(`${name}#L${row.line}: skipped — period "${period}" is not YYYY-M, YYYY-MM, or YYYY-MM-DD`);
      continue;
    }
    const num = (key, required = false) => {
      const v = cell(row, map, key).replace(/,/g, '');
      if (v === '') {
        if (required) warnings.push(`${name}#L${row.line}: skipped — ${key} is required`);
        return null;
      }
      const n = Number(v);
      if (!Number.isFinite(n)) {
        warnings.push(
          required
            ? `${name}#L${row.line}: skipped — ${key} "${cell(row, map, key)}" is not a non-negative integer`
            : `${name}#L${row.line}: ignored non-numeric ${key} "${cell(row, map, key)}"`
        );
        return null;
      }
      if (!Number.isInteger(n) || n < 0) {
        warnings.push(
          required
            ? `${name}#L${row.line}: skipped — ${key} "${cell(row, map, key)}" is not a non-negative integer`
            : `${name}#L${row.line}: ignored invalid ${key} "${cell(row, map, key)}" (must be a non-negative integer)`
        );
        return null;
      }
      return n;
    ...
    rows.push({ line: row.line, period, periodKey, activeUsers, events });
  }
  return {
    present: true,
    empty: rows.length === 0,
