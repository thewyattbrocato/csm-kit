...
function resolveColumns(header, headerLine, fileName, spec) {
  ...
  for (const canonical of Object.keys(spec)) {
    // `closed` is optional everywhere; everything else is part of the schema.
    if (!(canonical in map) && canonical !== 'closed') missing.push(canonical);
  }
  if (missing.length > 0) {
    throw new LoadError(
...
const SEVERITY_MAP = [
  [/^(p1|sev1|sev-1|1|critical|blocker|urgent|high)$/i, 'high'],
...
function loadTickets(filePath) {
  ...
  for (const row of table.rows) {
    ...
    const open = closedIso === null && (statusRaw === '' || isOpenStatus(statusRaw));
    rows.push({
      line: row.line,
      opened: isoOf(openedDt),
      openedDt,
      closed: closedIso,
      severityRaw: cell(row, map, 'severity'),
      severity: normalizeSeverity(cell(row, map, 'severity')),
...
function loadUsage(filePath) {
  ...
  for (const row of table.rows) {
    ...
    rows.push({ line: row.line, period, activeUsers: num('active_users'), events: num('events') });
  }
  return {
    present: true,
    empty: rows.length === 0,
